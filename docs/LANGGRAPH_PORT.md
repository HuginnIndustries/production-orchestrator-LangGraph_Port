...
| Resume | `agent([{"interruptResponse": {"interruptId": …, "response": "y" | "n"}}])` | `graph.invoke(Command(resume={"approved", "actor", "reviewed_hash"}), config)` — a bare `"y"`/`"n"` string is also accepted for parity |
...
| Identity | `record_decision(actor=…)` records the operator; `restart_spike` binds session and agent identity into the checkpoint and verifies them on resume | `RuntimeBinding{actor, provider, model_id}` is stored in thread state at `start`; the resuming process's binding must equal it and the decision's `actor` must equal the bound actor (`test_wrong_actor_is_refused`, `test_cli_refuses_a_swapped_actor_before_mutation`) |
| Provider binding | `_provider_configuration` persisted in the checkpoint and compared field-by-field at resume (provider, model, profile, region, credential source, Ollama host) | The same comparison on `provider` and `model_id` (`test_wrong_provider_binding_is_refused`, `test_wrong_model_binding_is_refused`, `test_cli_refuses_a_swapped_provider_binding`). There is no AWS profile / region / credential-source dimension because the LangGraph path has no cloud provider (see gaps) |
| Persisted decision is the one this thread made | n/a (single process records and applies in one hook) | `_verify_and_apply` step 4 requires `latest_decision(hash)` to match the actor, reviewed hash, reason, and approval the gate node recorded (`test_persisted_decision_mismatch_is_refused`) |
...
The 13 committed cases cover all three scenarios with both verdicts, quantity/priority/day variants that produce distinct hashes, an infeasible batch that the planner refuses, and two invalid intakes.
